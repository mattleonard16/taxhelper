/**
 * Receipt Job Confirm API Endpoint
 * POST /api/receipts/jobs/[id]/confirm - Confirm a receipt and create transaction
 */
...
import { confirmJob } from "@/lib/receipt/receipt-jobs-service";
...
export async function POST(
  ...
  try {
    ...
    const result = await confirmJob(user.id, id);
    ...
    logger.info("Receipt job confirmed", {
      requestId,
      userId,
      jobId: id,
      transactionId: result.data.transactionId,
    ...
    logger.error("Error confirming receipt job", {
      requestId,
      userId,
